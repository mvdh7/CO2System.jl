...
#
# Converted to Julia by Matthew P. Humphreys [2019-02-28]

println(" ")
println("This is an example of the use of CO2SYS.m")
println("that uses its ability to process vectors of data.")
println(" ")
println("We will generate a figure that shows the sensitivity of pH and pCO2")
println(" to changes in DIC, while keeping everything else constant")
# println(" ")
...
sp1 = plot(par2, A[:,4], color=:red, label="", xlabel="DIC", ylabel="pCO2 [uatm]",
    marker=(:circle, stroke(:red)))

# The calculated pH's are in the 3th column of the output A of CO2SYS
sp2 = plot(par2, A[:,3], color=:red, label="", xlabel="DIC", ylabel="pH",
    marker=(:circle, stroke(:red)))
